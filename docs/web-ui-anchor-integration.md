...
✅ Signing and sending transactions to your Anchor program
✅ Displaying on-chain data (balances, stake, rewards)

📌 Core Concepts

You must wrap your UI with a wallet context (ConnectionProvider, WalletProvider, WalletModalProvider).

Use Anchor’s TypeScript client (based on @coral-xyz/anchor) to *
...
2️⃣ Wallet Context Provider (React)
...
4️⃣ React UI: Connect Wallet + Display Info
...
5️⃣ React Hooks to Call Your Program
...
You’ll need to fill in the PDAs and token account addresses generated by your program. You can derive them via Anchor PDAs or use a client side helper.

6️⃣ Claim Rewards Button
...
🔹 Always wrap your UI with Solana Wallet context providers. 
Solana

🔹 Use Anchor’s getProgram() wrapper to cleanly interact with your on-chain program.
🔹 Validate PDAs and token account addresses — mismatches lead to transaction errors.
🔹 Consider adding pending/loading UI state (e.g., spinners) when sending RPCs.
🔹 Test on Devnet before moving to Mainnet.
